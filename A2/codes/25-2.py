...
def write_to_file(generation, error, fitness,iter,type):
    ...
    with open('../output_files/25-2.txt', 'a+') as write_file:
        write_file.write((str('ITERATION  :  ') + str(iter) + str('\n\n')))
...
def write_to_file_best(best_error,best_gen,best_fitness,iter):
    ...
    iter += 1
    with open('../output_files/25-final.txt', 'a+') as write_file:
...
def create_new_gen(generation,mutated_generation,error,child_error,fitness,child_fitness):
    ...
    pot_new_generation_error = np.concatenate((error[:5],child_error), axis = 0)


    top10,top10error,top10fitness,sorted_idx = sort_generation(pot_new_generation, pot_new_generation_error, pot_new_generation_fitness)



    new_generation = top10[:10]
    new_error = top10error[:10]
...
def range_calc_expand(MUTATE_RANGE,iter):
    curr_range = MUTATE_RANGE
    if(iter < 5):
        curr_range[0] -= (iter) * 0.01
        curr_range[1] += (iter)* 0.01
    else :
        curr_range[0] = 0.8
        curr_range[1] = 1.2
...
def range_calc_shrink(MUTATE_RANGE,iter):
...
def main_loop1(delta):
    ...
    with open('../output_files/25-2.txt', 'a+') as write_file:
        ...
        write_file.write((str('RUN TYPE  :  ') + str(a) + str('\n\n\n\n\n')))
    ...
    for iter in range(ITERATIONS):
        ...
        if(delta == 0):
            curr_range = range_calc_expand(MUTATE_RANGE,iter)
        if(delta == 1):
            curr_range = range_calc_shrink(MUTATE_RANGE,iter)
        ...
        if(iter == ITERATIONS - 1):
            ...
            if(delta == 0):
                plt.savefig('../output_files/expand-25.jpeg')
            if(delta == 1):
                plt.savefig('../output_files/shrink-25.jpeg')
...
##CALL FIRST ITERATION (EXPAND MUTATION RANGE)

gen1 , fitness1, error1 = main_loop1(0)



##CALL FIRST ITERATION (SHRINK MUTATION RANGE)

gen2 , fitness2, error2 = main_loop1(1)
