...
overfit_vector = [ 0.00000000e+00 ,-1.51471993e-12, -2.22066211e-13 , 6.48416136e-11 ,-1.56394169e-10, -1.42959903e-15 , 7.44095225e-16 , 2.68514985e-05 ,  -1.74755002e-06 , -1.47003951e-08 , 7.74706483e-10]

def get_prev():

    f = open('../output_files/4-3/endVec.json')
    data = json.load(f)
...
def write_to_json(generation,error,fitness):
    ...
    with open('../output_files/4-3/endVec.json','w') as f:
...
def write_to_file(generation, error, fitness,iter,type):
    ...
    with open('../output_files/4-3/4-3.txt', 'a+') as write_file:
...
def write_to_file_best(min_error1,min_error2,min_vector,min_fitness,iterations):
    ...
    with open('../output_files/4-3/4-3-final.txt', 'a+') as write_file:
        # write_file.write((str('\n\nITERATION  :  ') + str(iter) + str('\n\n')))
        write_file.write(table)
...
def fitness_function(error):
    ...
    for i in range(len(error)):
        ...
        fitness[i] = (( (1/training) + validation ))
...
def main_loop():
    ...
    with open('../output_files/4-3/4-3.txt', 'a+') as write_file:
        write_file.write(str('\n\n') +str(line)+ str('\n\n'))
        write_file.write((str('GENERATIONS: 10 | ITERATIONS: 6 | RUN NUMBER: 10') + str('\n\n\n\n\n')))
    ...
    for iter in range(ITERATIONS):
        ...
        if(iter == ITERATIONS - 1):
            ...
            plt.ylabel("errors")
            plt.savefig('../output_files/4-3/expand1.jpeg') 
